...
functions are [widget.NewButton()](/api/v2/widget/button/#func--newbutton) and [widget.NewButtonWithIcon()](/api/v2/widget/button/#func--newbuttonwithicon).
...
which is the [fyne.Resource](/api/v2/fyne/resource/#type--resource) which contains the icon data.
...
as a resource - helpers such as [fyne.LoadResourceFromPath()](/api/v2/fyne/resource/#func--loadresourcefrompath) may assist,
though bundling resources is recommended where possible.

To create a button with only an icon you should pass "" as the label
parameter to [widget.NewButtonWithIcon()](/api/v2/widget/button/#func--newbuttonwithicon).
