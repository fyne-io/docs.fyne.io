...
A widget is split into two areas - each implementing a standard interface - the [fyne.Widget](/api/v2/fyne/widget/#type--widget) and the [fyne.WidgetRenderer](/api/v2/fyne/widgetrenderer/#type--widgetrenderer). The widget defines behaviour and state, with the renderer being used to define how it should be drawn to screen.

### fyne.Widget

A widget in Fyne is simply a stateful canvas object that has its rendering
definition separated from the main logic. As you can see from the [fyne.Widget](/api/v2/fyne/widget/#type--widget)
...
To save writing all the functions required we can make use of the [widget.BaseWidget](/api/v2/widget/basewidget/#type--basewidget) type which handles the basics.
...
For example, look at the [widget.Button](/api/v2/widget/button/#type--button) type:
...
The widget renderer is responsible for managing a list of [fyne.CanvasObject](/api/v2/fyne/canvasobject/#type--canvasobject) primitives that come together to create the design of our widget. It is much like
a [fyne.Container](/api/v2/fyne/container/#type--container) with a custom layout and some additional theme handling.
...
[fyne.Layout](/api/v2/fyne/layout/#type--layout) interface, without the `[]fyne.CanvasObject` parameter - this is because a widget does need to be laid out but it controls which objects will be included.
...
Compare again with the button widget - it's [fyne.WidgetRenderer](/api/v2/fyne/widgetrenderer/#type--widgetrenderer) implementation is based on the following type:
...
objects for drawing.
It keeps track of the slice of objects required by [fyne.WidgetRenderer](/api/v2/fyne/widgetrenderer/#type--widgetrenderer) as a convenience.

Lastly it keeps a reference to the [widget.Button](/api/v2/widget/button/#type--button) for all state information.
In the `Refresh()` method it will update the graphical state based on any changes
in the underlying [widget.Button](/api/v2/widget/button/#type--button) type.

### Bring it together

A basic widget will extend the [widget.BaseWidget](/api/v2/widget/basewidget/#type--basewidget) type and declare any state
that the widget holds.
The `CreateRenderer()` function must exist and return a new [fyne.WidgetRenderer](/api/v2/fyne/widgetrenderer/#type--widgetrenderer) instance. The widget and driver code in Fyne will ensure that this
